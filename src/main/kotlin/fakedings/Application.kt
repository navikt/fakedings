...
internal fun ok(body: String?) = response(200, Headers.headersOf(), body)
...
internal fun RecordedRequest.toPath(): String? = this.requestUrl?.pathSegments?.joinToString("/", "/")

internal fun RecordedRequest.param(name: String): String? = this.requestUrl?.queryParameter(name)

internal infix fun RecordedRequest.shouldHavePath(path: String): Boolean = this.requestUrl?.pathSegments == path.toPathSegments()
...
object Config {
    val host: InetAddress =
        "OAUTH2_SERVER_HOST".fromEnv()?.let {
